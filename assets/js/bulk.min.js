...
a.percent+"</span>%"),m(b("#attachment-"+a.image+" .imagify-cell-percentage").find("canvas")),b("#attachment-"+a.image+" .imagify-cell-thumbnails").html(a.thumbnails),b("#attachment-"+a.image+" .imagify-cell-savings").html(Optimizer.toHumanSize(a.overall_saving,1)),d=d+a.thumbnails+1,b(".imagify-cell-nb-files").html(d+" file(s)"),f+=a.original_overall_size,b(".imagify-total-original").html(Optimizer.toHumanSize(f,1)),g+=a.overall_saving,b(".imagify-total-gain").html(Optimizer.toHumanSize(g,1))):(error_class=
"error",error_dashicon="dismiss",error_message="Error",0<=a.error.indexOf("You've consumed all your data")&&swal({title:imagifyBulk.overQuotaTitle,text:imagifyBulk.overQuotaText,type:"error",customClass:"imagify-sweet-alert",html:!0},function(){location.reload()}),0<=a.error.indexOf("This image is already compressed")?(error_dashicon=error_class="warning",error_message="Notice"):(l++,b(".imagify-cell-errors").html(l+" error(s)")),b("#attachment-"+a.image).after('<tr><td colspan="7"><span class="status-'+
...
imagifyBulk.pluginURL+"&amp;text="+text2share+"&amp;url="+imagifyBulk.pluginURL+"&amp;related=imagify&amp;hastags=performance,web,wordpress"),b(".imagify-ac-chart").attr("data-percent",a.global_percent),n(b(".imagify-ac-chart").find("canvas")))}).error(function(a){k.log("Can't optimize image with id "+a)}).run()}else c.removeAttr("disabled"),c.find(".dashicons").removeClass("rotate"),b(window).off("beforeunload",confirmMessage),k.log(a),"over-quota"==a.data.message&&swal({title:imagifyBulk.overQuotaTitle,
