...
1)),k+=b.overall_saving,a(".imagify-total-gain").html(Optimizer.toHumanSize(k,1))):(a("#attachment-"+b.image).after('<tr><td colspan="7"><span class="status-error">'+b.error+"</span></td></tr>"),a("#attachment-"+b.image+" .imagify-cell-status").html('<span class="imagistatus status-error"><span class="dashicons dashicons-dismiss"></span>Error</span>'),m++,a(".imagify-cell-errors").html(m+" error(s)"));f.segments[0].value=b.global_unoptimized_attachments;f.segments[1].value=b.global_optimized_attachments;
f.segments[2].value=b.global_errors_attachments;f.update()}).done(function(b){d.removeAttr("disabled");d.find(".dashicons").removeClass("rotate");a(window).off("beforeunload",confirmMessage);a(".imagify-row-progress").slideUp();"NaN"!==b.global_percent&&(a(".imagify-row-complete").removeClass("hidden").addClass("done").attr("aria-hidden","false"),a("html, body").animate({scrollTop:a(".imagify-row-complete").offset().top},200),a(".imagify-ac-rt-total-gain").html(b.global_gain_human),a(".imagify-ac-rt-total-original").html(b.global_original_size_human),
text2share=imagifyBulk.textToShare,text2share=text2share.replace("%1$s",b.global_gain_human),text2share=text2share.replace("%2$s",b.global_original_size_human),text2share=encodeURIComponent(text2share),a(".imagify-sn-twitter").attr("href","https://twitter.com/intent/tweet?source=webclient&amp;original_referer="+imagifyBulk.pluginURL+"&amp;text="+text2share+"&amp;url="+imagifyBulk.pluginURL+"&amp;related=imagify&amp;hastags=performance,web,wordpress"),a(".imagify-ac-chart").attr("data-percent",b.global_percent),
p(a(".imagify-ac-chart").find("canvas")))}).error(function(a){l.log("Can't optimize image with id "+a)}).run()}else d.removeAttr("disabled"),d.find(".dashicons").removeClass("rotate"),a(window).off("beforeunload",confirmMessage),l.log(c),"over-quota"==c.data.message&&swal({title:imagifyBulk.overQuotaTitle,text:imagifyBulk.overQuotaText,type:"error",customClass:"imagify-sweet-alert",html:!0}),"no-images"==c.data.message&&swal({title:imagifyBulk.noAttachmentToOptimizeTitle,text:imagifyBulk.noAttachmentToOptimizeText,
type:"info",customClass:"imagify-sweet-alert"})})})});var width=700,height=290;if(window.innerWidth)var clientLeft=(window.innerWidth-width)/2,clientTop=(window.innerHeight-height)/2;else clientLeft=(document.body.clientWidth-width)/2,clientTop=(document.body.clientHeight-height)/2;
