...
{value:imagifyBulk.totalErrorsAttachments,color:"#2E3242",highlight:"#2E3242",label:imagifyBulk.overviewChartLabels.error}];if(d){var f=(new Chart(d.getContext("2d"))).Doughnut(q,{segmentStrokeColor:"transparent",segmentStrokeWidth:0,animateRotate:!0,animation:!0,percentageInnerCutout:85,legendTemplate:'<ul class="imagify-<%=name.toLowerCase()%>-legend"><% for (var i=0; i<segments.length; i++){%><li><span style="background-color:<%=segments[i].fillColor%>"></span><%if(segments[i].label){%><%=segments[i].label%><%}%></li><%}%></ul>',
tooltipTemplate:"<%= value %>"}),d=f.generateLegend();document.getElementById("imagify-overview-chart-legend").innerHTML=d}a("#imagify-simulate-bulk-action").click(function(c){c.preventDefault();a("#imagify-bulk-action").trigger("click")});a("#imagify-bulk-action").click(function(){var c=a(this),d=a('[name="optimization_level"]:checked').val();if(c.attr("disabled"))return!1;c.attr("disabled","disabled");c.find(".dashicons").addClass("rotate");confirmMessage=function(){return"Imagify is processing. Are your sure you want to quit this page?"};
a(window).on("beforeunload",confirmMessage);a.get(ajaxurl+"?action=imagify_get_unoptimized_attachment_ids&optimization_level="+d+"&imagifybulkuploadnonce="+a("#imagifybulkuploadnonce").val()).done(function(e){if(e.success){e={lib:ajaxurl+"?action=imagify_bulk_upload&imagifybulkuploadnonce="+a("#imagifybulkuploadnonce").val(),images:e.data};var d=a(".imagify-bulk-table table tbody"),g=0,m=0,h=0,k=0;a(".imagify-row-progress").slideDown();a(".imagify-no-uploaded-yet, .imagify-row-complete").hide(200);
Optimizer=new ImagifyGulp(e);Optimizer.before(function(a){d.append('<tr id="attachment-'+a.id+'"><td class="imagify-cell-filename"><span class="imagiuploaded"><img src="'+a.thumbnail+'"/>"</span><span class="imagifilename">'+a.filename+'</span></td><td class="imagify-cell-status"><span class="imagistatus status-compressing"><span class="dashicons dashicons-admin-generic rotate"></span>Compressing<span></span></span></td><td class="imagify-cell-original"></td><td class="imagify-cell-optimized"></td><td class="imagify-cell-percentage"></td><td class="imagify-cell-thumbnails"></td><td class="imagify-cell-savings"></td></tr>')}).each(function(b){var c=
...
error_class+'"><span class="dashicons dashicons-'+error_dashicon+'"></span>'+error_message+"</span>"));f.segments[0].value=b.global_unoptimized_attachments;f.segments[1].value=b.global_optimized_attachments;f.segments[2].value=b.global_errors_attachments;f.update()}).done(function(b){c.removeAttr("disabled");c.find(".dashicons").removeClass("rotate");a(window).off("beforeunload",confirmMessage);a(".imagify-row-progress").slideUp();"NaN"!==b.global_percent&&(a(".imagify-row-complete").removeClass("hidden").addClass("done").attr("aria-hidden",
