	...
	<td class="imagify-cell-checkbox">
		<p>
			<span class="imagify-cell-checkbox-box">
				<input id="cb-select-<?php echo $data['folder_type']; ?>" type="checkbox" name="group[]" checked="checked" value="<?php echo $data['folder_type']; ?>" />
		...
		<div class="imagify-level-selector">
			...
			<ul id="imagify-<?php echo $data['folder_type']; ?>-level-selector-list" role="listbox" aria-orientation="vertical" aria-hidden="true" class="imagify-level-selector-list">
				...
				<li class="imagify-level-choice<?php echo $level === $default_level ? ' imagify-current-level" aria-current="true' : ''; ?>" role="option">
					...
					<label for="<?php echo $data['folder_type']; ?>-level-<?php echo $level; ?>"><?php echo imagify_get_optimization_level_label( $level, '%ICON% %s' ); ?></label>
				</li>
				<?php } ?>
			</ul>
		</div>
