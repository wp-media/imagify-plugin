...
**Never hardcode a URL, port, or credential — everything comes from `.ai/settings.local.json`**
(gitignored; generated by `bin/qa-env.sh up`, or hand-written for Local-site setups). Shape:

```json
{
  "environments": {
    "nginx":  { "url": "...", "username": "...", "password": "...", "wp_cli": "...", "wp_path": "..." },
    "apache": { "url": "...", "username": "...", "password": "...", "wp_cli": "...", "wp_path": "..." }
  },
  "imagify":  { "api_key": "..." },
...
Each env block carries `wp_cli` (full command prefix — qa-env style) **or** `wp_path`
(Local style → prefix is `wp --path=<wp_path>`). All examples below write `$E2E_URL` and
`$WPCMD` — resolve them from the config for the case's target env before use.
