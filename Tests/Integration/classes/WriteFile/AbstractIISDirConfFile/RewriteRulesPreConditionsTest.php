...
class RewriteRulesPreConditionsTest extends TestCase {
	...
	/**
	 * Rewriting a legacy file with separate wrappers must converge to one shared container.
	 */
	public function testLegacySeparateContainersAreConverged() {
		$this->seed( '<configuration><system.webServer><rewrite><rules/><outboundRules><preConditions name="Imagify: rewrite rules for webp 4"><preCondition name="IsWebp"/></preConditions><preConditions name="Imagify: rewrite rules for avif 4"><preCondition name="IsAvif"/></preConditions></outboundRules></rewrite></system.webServer></configuration>' );
	...
	/**
	 * Re-adding one format must also merge both legacy wrappers.
	 */
	public function testSingleFormatAddConvergesLegacySeparateContainers() {
		$this->seed( '<configuration><system.webServer><rewrite><rules/><outboundRules><preConditions name="Imagify: rewrite rules for webp 4"><preCondition name="IsWebp"/></preConditions><preConditions name="Imagify: rewrite rules for avif 4"><preCondition name="IsAvif"/></preConditions></outboundRules></rewrite></system.webServer></configuration>' );
