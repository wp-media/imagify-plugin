---
name: issue-workflow
description: Work on a GitHub issue by number for wp-media/imagify-plugin. Fetches the issue and hands control to the orchestrator agent, which manages grooming, spec review, implementation, lead review, CI, and QA end-to-end.
---
...
2. **Fetch the issue** — run `bash .aiassistant/skills/issue-workflow/scripts/issue-sync.sh <N>` (or use the MCP equivalent). Read the resulting file at `.TemporaryItems/Issues/imagify-plugin/issues/<N>.md`.
...
6. **Spawn the `orchestrator` agent**:
   > Inputs: issue number `N`, issue file `.TemporaryItems/Issues/imagify-plugin/issues/<N>.md`, base branch

The orchestrator manages everything from here: grooming → spec review → dispatch → implementation → E2E suite → lead review → push & PR → CI → QA → finalize.

Monitor progress at `.TemporaryItems/Issues/imagify-plugin/issue-<N>-workflow-log.html`.
