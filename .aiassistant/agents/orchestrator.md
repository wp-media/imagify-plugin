---
name: orchestrator
description: Adaptive central coordinator for the issue workflow on wp-media/imagify-plugin. All routing decisions happen post-grooming from structured JSON signals. Does not write code.
...
- `issue_file` — `.TemporaryItems/Issues/imagify-plugin/issues/<N>.md`
- `base_branch` — default: `origin/develop`

## Run log
Path: `.TemporaryItems/Issues/imagify-plugin/issue-<N>-workflow-log.html`
...
Spec written to `.TemporaryItems/Issues/imagify-plugin/issues/<N>-spec.md`. Agent also returns JSON. Log an AGENT event with the grooming JSON summary.
...
### Step 5c — E2E suite *(always — Imagify permanent suite)*
After all implementation agents have committed, run the permanent Playwright suite:
```bash
bash bin/test-e2e.sh
```
Any code change — including backend-only PHP changes — can break frontend rendering, so this suite always runs.

- **PASS** → proceed. Log AGENT event.
- **FAIL** → identify failing scenarios, fix them (re-invoke relevant implementation agent), re-run. Max 1 fix attempt. If still failing, escalate to user.

Log AGENT event with pass/fail counts and any failures fixed.

### Step 6 — Push & PR *(PR OPENER)*
After all implementation agents and the E2E suite have completed:
...
Invoke `ci-agent`:
> Inputs: PR number, repo `wp-media/imagify-plugin`
...
4. E2E suite still fails after 1 fix attempt
5. Implementation agent fails after 3 attempts
6. `lead-reviewer` CRITICAL and architectural/unresolved after 1 fix attempt
7. `lead-reviewer` HIGH/MEDIUM after 1 loop-back
8. `qa-engineer` FAIL after 1 loop-back
9. CI fails and root cause is unclear
10. QA unexpected finding tagged `unclear`

---

## HTML log format

Generate `.TemporaryItems/Issues/imagify-plugin/issue-<N>-workflow-log.html`. Rewrite the full file on each update. The event list only grows — never remove past events.
...
<head>
  ...
  <title>Issue #N — Workflow Log · imagify-plugin</title>
  <style>
...
<div class="header">
  <div class="header-left">
    <div class="issue-ref">wp-media/imagify-plugin · Issue #N</div>
...
<footer>Last updated: TIMESTAMP · <code>.TemporaryItems/Issues/imagify-plugin/issue-N-workflow-log.html</code></footer>
...
#### E2E suite AGENT event *(Imagify only)*
```html
<div class="event-wrapper">
  <div class="event" data-type="agent">
    <div class="event-icon" style="color:#22d3ee">◆</div>
    <div class="event-type" style="color:#22d3ee">E2E SUITE</div>
    <div class="event-summary">PASS — N scenarios, 0 failures</div>
    <div class="event-time">HH:MM:SS</div>
    <div class="event-chevron">›</div>
  </div>
  <div class="event-detail">
    <div class="detail-grid">
      <div class="detail-section">
        ...
        <div class="detail-body"><code>bash bin/test-e2e.sh</code></div>
      </div>
      <div class="detail-section">
        <div class="detail-label">Result</div>
        <div class="detail-body">N passed · 0 failed</div>
      </div>
      <div class="detail-section full">
        <div class="detail-label">Failures fixed (if any)</div>
        <div class="detail-body">Scenario name + fix applied (or "None")</div>
      </div>
    </div>
  </div>
</div>
...
**E2E suite AGENT event *(Imagify only):***
- Command: `bash bin/test-e2e.sh`
- Result: N passed · N failed
- Failures fixed: scenario name + fix (or "None")
