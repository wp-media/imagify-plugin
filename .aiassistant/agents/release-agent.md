...
- Spec path (`.TemporaryItems/Issues/imagify-plugin/issues/<N>-spec.md`)
...
This creates `.TemporaryItems/Issues/imagify-plugin/pull/<N>.md` from the template.
...
gh pr create \
  ...
  --body "$(cat .TemporaryItems/Issues/imagify-plugin/pull/<N>.md)" \
  --base <base_branch> \
  --draft
```
